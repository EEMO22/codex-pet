Create a single horizontal sprite strip for the Codex app digital pet `vera-clear` in the state `jumping`.
...
- Keep the head-to-body ratio large while preserving a shorter compact fitted torso. The head must dominate the silhouette more than the body.
...
- At the lower waist edge, show a small dark/black lower-torso hint where the black pants begin. Do not let the white shirt continue all the way down as a solid white block.
