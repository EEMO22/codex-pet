Create a single horizontal sprite strip for the Codex app digital pet `vera-clear` in the state `running-right`.
...
Animation action: rightward pointing travel pose with hair flowing left.
...
- The pet faces toward the right. Hair must flow/trail left, opposite the travel direction.
- Extend the right arm toward the right as if pointing where to go. Keep the arm readable but inside each frame slot.
- Make the row feel like rightward movement through upper-body bob, shoulder shift, hair flow, and pointing-arm rhythm, not through legs.

State-specific requirements:
- Show rightward locomotion through face direction, body bob, right arm pointing right, and hair trailing left.
- Do not draw speed lines, dust clouds, floor shadows, motion trails, or detached motion effects.
- Do not turn this into full-body foot running; keep it bust-only, compact, and no visible legs.
...
Repair attempt 1:
- The previous `running-right` strip failed QA: Previous running-right row interpreted the motion as a generic side-facing upper-body run. It drifted from Vera Clear's canonical character and did not match the vera-reviewer movement: facing/rightward gaze progression with hair swept opposite the travel direction.
