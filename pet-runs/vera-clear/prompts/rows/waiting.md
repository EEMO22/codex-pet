Create a single horizontal sprite strip for the Codex app digital pet `vera-clear` in the state `waiting`.
...
Animation action: bored waiting at laptop with chin resting on one hand.
...
- The pet rests her chin on one hand and looks bored while waiting behind or beside a dark charcoal/black laptop.
- The laptop lid must show one very large centered high-contrast light OpenAI knot logo based on references/openai-logo-reference-on-white.png.
- Match the laptop size, laptop angle, character scale, and overall character-to-laptop proportions from the approved `running` row reference. Waiting should feel like the exact same desk setup as running, only with a bored chin-on-hand pose.
- Use the same diagonal laptop placement as running, with the character beside/behind the laptop at the same scale. Do not make the laptop a large straight-on rectangle.
- Use a bored expression, slow blink, small head tilt, and tiny idle movement only.

State-specific requirements:
- Show patient boredom: chin propped on one hand, bored eyes or slow blink, small head tilt, and minimal movement.
- Keep the laptop inside the frame and logo readable at final app scale; do not add text or extra UI on the laptop.
- Do not make the laptop much larger, more front-facing, or differently angled than the approved running row. Keep the character size and head/body proportion close to running.
...
- Do not add floating symbols, ellipses, UI, scenery, shadows, or detached effects.
