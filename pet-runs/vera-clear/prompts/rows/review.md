Create a single horizontal sprite strip for the Codex app digital pet `vera-clear` in the state `review`.
...
Animation action: serious arms-crossed review at laptop with a more visible head-tilt loop.
...
- Place a dark charcoal/black laptop in front of the pet. The lid must show one very large centered high-contrast light OpenAI knot logo based on references/openai-logo-reference-on-white.png.
- The pet crosses her arms, makes a serious expression, and tilts her head as if reviewing something critically.
- Make the head tilt more visible than the previous version: alternate neutral, tilted left, deeper tilted left, tilted right or returning, then settle.
- Match the approved `running` row's laptop size, laptop angle, character scale, and character-to-laptop placement. The review laptop should use the same diagonal desk/laptop language as running, not a huge straight-on front wall.
- Keep the character beside/behind the laptop in the same proportion as running; the laptop must not cover too much of the torso or dominate the frame more than running does.
- Keep the body and laptop stable while the head/neck angle does the main motion. Do not type in this row.

State-specific requirements:
- Show review through crossed arms, serious eyes, slight frown or focused mouth, and a clearly readable head-tilt/gawking motion near the laptop.
- The head tilt must read at actual app size; avoid only a tiny eye blink with no visible head angle change.
- Keep the logo readable at final app scale; prefer a bold simplified knot over tiny exact linework. Do not add text, code, papers, UI, punctuation, symbols, or other new props.
- Use `decoded/running.png` only as a layout reference for laptop angle/size and character-laptop placement; do not copy its typing hand motion.
- Do not add magnifying glasses or extra review props.
