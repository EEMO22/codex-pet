Create a single horizontal sprite strip for the Codex app digital pet `vera-clear` in the state `running-left`.
...
Animation action: leftward pointing travel pose with hair flowing right.
...
- The pet faces toward the left. Hair must flow/trail right, opposite the travel direction.
- Extend the left arm toward the left as if pointing where to go. Keep the arm readable but inside each frame slot.
- Make the row feel like leftward movement through upper-body bob, shoulder shift, hair flow, and pointing-arm rhythm, not through legs.

State-specific requirements:
- Show leftward locomotion through face direction, body bob, left arm pointing left, and hair trailing right.
