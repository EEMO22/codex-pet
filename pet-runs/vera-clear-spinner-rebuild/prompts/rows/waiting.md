Create a single horizontal sprite strip for the Codex app digital pet `vera-clear-spinner` in the state `waiting`.
...
Final Vera Clear Spinner waiting override:
...
- Vera rests her chin on one hand, looks slightly bored, and watches the laptop screen.
- Across 6 frames: chin-rest gaze at laptop, eyelids droop, blink, brief eyes-closed bored pause, eyes reopen, settle into the same bored laptop-watching pose.
- Keep the chin-rest hand, laptop size/angle, character scale, and current spinner waiting row composition, but match idle's line style and face proportions.

Repair attempt 2:
- The previous waiting strip failed QA because the loading spinner did not read as a clean clockwise progression and the head tilt stayed too strongly to one side.
- Regenerate the whole row as a symmetric bored review loop: frame 1 center, frame 2 slight tilt to Vera's left, frame 3 blink/center, frame 4 slight tilt to Vera's right, frame 5 eyes reopen through center, frame 6 settle. Do not make all tilt frames lean the same way.
- The spinner must be the simple browser-style loader used on the laptop lid: one segmented gray ring with exactly one bright white arc/head. Across the 6 frames, the bright arc/head advances clockwise in even steps. Do not draw two bright heads, a back-and-forth swing, a knot logo, a complete white circle, text, UI, or a brand mark.
- Keep Vera and the laptop nearly identical in size to the `running` and `review` laptop rows: same visible character height, same laptop width, same lid angle, same laptop vertical placement, and same screen/lid thickness.
- Preserve the accepted Vera identity: same face width as idle/waving, same rounded black bob hair, same white V-neck shirt, same outline weight, same flat sprite rendering.

Repair attempt 3:
- The previous waiting strip failed user QA because the head tilt was too exaggerated and Vera looked toward the viewer instead of the laptop.
- Waiting is a quiet bored laptop-watching pose, not a playful head-sway animation. Keep the head almost centered with only tiny left/right micro-tilts.
- Vera's gaze must point down toward the laptop screen/keyboard area in every frame. Do not make eye contact with the viewer. Use lowered pupils, slightly lowered eyelids, or eyes-closed blink frames to show she is watching/waiting on the laptop.
