Create a single horizontal sprite strip for the Codex app digital pet `vera-clear-spinner` in the state `running-right`.
...
Final Vera Clear Spinner running-right override:
- No laptop, no glasses, no props, no detached effects.
- Vera turns her body slightly toward the movement direction and points to the right with one hand.
- Hair flows opposite the movement direction, trailing left, but stays close enough to preserve the base silhouette and avoid slot crossing.
- Across 8 frames, keep a subtle directional travel loop: point right, tiny torso lean right, hair/hand timing shifts, then cycle back.
- This row and running-left must be near-mirrors in rhythm, scale, body angle, face size, and line style. Differences should only express direction.

Repair attempt 2:
- The previous running-right strip failed QA because the wind-swept hair read too long, closer to mid-back length than Vera's shoulder-length bob.
...
- Preserve the same face size, body scale, shirt shape, outline weight, and sprite rendering as idle/waving. No laptop or added prop.
