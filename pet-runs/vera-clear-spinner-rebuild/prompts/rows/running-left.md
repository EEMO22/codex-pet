Create a single horizontal sprite strip for the Codex app digital pet `vera-clear-spinner` in the state `running-left`.
...
- No laptop, no glasses, no props, no detached effects.
- Vera turns her body slightly toward the movement direction and points to the left with one hand.
- Hair flows opposite the movement direction, trailing right, but stays close enough to preserve the base silhouette and avoid slot crossing.
- Across 8 frames, keep a subtle directional travel loop: point left, tiny torso lean left, hair/hand timing shifts, then cycle back.
- Match running-right as closely as possible in rhythm, scale, body angle, face size, and line style. Differences should only express direction.
