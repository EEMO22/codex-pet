Create a single horizontal sprite strip for the Codex app digital pet `vera-clear-spinner` in the state `waving`.
...
Final Vera Clear Spinner waving override:
- No laptop, no glasses, no floating kiss mark, no wave marks, no detached effects.
